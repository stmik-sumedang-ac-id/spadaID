...
namespace tool_spadaindonesia\form;
...
class edit extends moodleform {
     ...
     * @var \tool_spadaindonesia\record_interface
     */
    protected $record;

    /**
     * Web analytics tool of provided type.
     * @var \tool_spadaindonesia\tool\tool_interface
     */
    protected $tool;
    ...
    public function definition() {
        ...
        $mform->addElement('select', 'type', get_string('type', 'tool_spadaindonesia'), $choices);
        $mform->setType('type', PARAM_TEXT);

        $mform->addElement('checkbox', 'enabled', get_string('enabled', 'tool_spadaindonesia'));
        $mform->addHelpButton('enabled', 'enabled', 'tool_spadaindonesia');
        $mform->setDefault('enabled', 1);

        $mform->addElement('text', 'name', get_string('name', 'tool_spadaindonesia'));
        $mform->addHelpButton('name', 'name', 'tool_spadaindonesia');
        $mform->setType('name', PARAM_TEXT);
        $this->_form->addRule('name', get_string('required'), 'required', null, 'client');

        $choices = array(
            'head' => get_string('head', 'tool_spadaindonesia'),
            'topofbody' => get_string('topofbody', 'tool_spadaindonesia'),
            'footer' => get_string('footer', 'tool_spadaindonesia'),
        );

        $mform->addElement('select', 'location', get_string('location', 'tool_spadaindonesia'), $choices);
        $mform->addHelpButton('location', 'location', 'tool_spadaindonesia');
        $mform->setType('location', PARAM_TEXT);

        $mform->addElement('checkbox', 'trackadmin', get_string('trackadmin', 'tool_spadaindonesia'));
        $mform->addHelpButton('trackadmin', 'trackadmin', 'tool_spadaindonesia');

        $mform->addElement('checkbox', 'cleanurl', get_string('cleanurl', 'tool_spadaindonesia'));
        $mform->addHelpButton('cleanurl', 'cleanurl', 'tool_spadaindonesia');
